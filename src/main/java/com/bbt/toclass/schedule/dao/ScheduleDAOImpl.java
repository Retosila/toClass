...
public class ScheduleDAOImpl implements ScheduleDAO {
	...
	@Override
	public List<MenuVO> getMenuInfoByDTO(MenuDTO menuDTO) throws Exception {
		System.out.println("DB에 getMenuInfoByDTO 요청");
		List<MenuVO> menuVOList = sqlSession.selectList("mapper.menu.getMenuInfoByDTO", menuDTO);
		logger.info("myBatis로부터 성공적으로 응답 수신 : " + menuVOList.size() + "개" );
		for (MenuVO mvo : menuVOList) {
			System.out.println(mvo.getMenu_content());
		}
		return menuVOList;
	}
	...
	@Override
	public List<MenuDTO> getMenuDate(Date menu_date) throws Exception {
		System.out.println("DB에 getMenuDate 요청");
		List<MenuDTO> menuDateList = sqlSession.selectList("mapper.menu.getMenuDate", menu_date);
		for(MenuDTO mdto : menuDateList) {
			System.out.println(mdto.getMenu_date());
		}
		return menuDateList;
	}
}
